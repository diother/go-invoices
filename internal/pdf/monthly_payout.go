...
func (p MonthlyPayoutPdf) addHeader(pdf *gopdf.GoPdf, issueDate string) error {
...
func (p MonthlyPayoutPdf) addSecondaryHeader(pdf *gopdf.GoPdf) error {
	const startY = marginTop
...
func (p MonthlyPayoutPdf) addFooter(pdf *gopdf.GoPdf, pagesNeeded int, currentPage int) error {
...
func (m MonthlyPayoutPdf) GenerateDocument() error {
	pdf := gopdf.GoPdf{}
	...
	err = m.addHeader(&pdf, m.MonthlyPayout.IssueDate)
	if err != nil {
		return err
	}
	err = m.addFooter(&pdf, pagesNeeded, currentPage)
	if err != nil {
		return err
	}
