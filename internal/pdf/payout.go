...
func (p PayoutPdf) addHeader(pdf *gopdf.GoPdf, issueDate string) error {
...
func (p PayoutPdf) addSecondaryHeader(pdf *gopdf.GoPdf) error {
	const startY = marginTop
...
func (p PayoutPdf) addFooter(pdf *gopdf.GoPdf, pagesNeeded int, currentPage int) error {
...
func (p PayoutPdf) GenerateDocument() error {
	pdf := gopdf.GoPdf{}
	...
	err = p.addHeader(&pdf, p.Payout.IssueDate)
	if err != nil {
		return err
	}
	err = p.addFooter(&pdf, pagesNeeded, currentPage)
	if err != nil {
		return err
	}
